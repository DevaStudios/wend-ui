...
description: Use when editing any file under packages/tokens/tokens/**/*.json (global, semantic, or component token JSON), when a component's token reference changes (e.g. which global/semantic token it aliases), or when checking Figma for drift/new content generally — always check Figma's local Styles (text/effect/paint/grid, via getLocalTextStylesAsync() etc.), not just Variables, since Styles are a separate Figma feature that silently accumulates unsynced content otherwise. Figma Variables, packages/tokens/README.md, and any other docs (including docs/superpowers/ plans and specs) that mention the old token names must be kept in sync with the token source, and won't update themselves.
---
...
- Any time you're checking Figma for drift or new content (a user asking "sync with Figma," "check for changes," etc.) — **always check Styles as well as Variables** (see step 2). Styles are a second, separate Figma feature (Text/Effect/Paint/Grid Styles) that every prior sync in this project only checked Variables for; nothing surfaces newly-added Styles on its own.
...
2. **Pull current state from both sides:**
   ...
   - **Figma Styles too** — a separate feature from Variables, with no `diff_tokens`-equivalent tool, so check by hand: `figma.getLocalTextStylesAsync()`, `getLocalEffectStylesAsync()`, `getLocalPaintStylesAsync()`, `getLocalGridStylesAsync()`, all read-only via `use_figma`. Each returned style's `boundVariables` tells you which existing Variables (hence which existing tokens) it's built from — a Style that's just a named bundle of properties you already track (e.g. a text style combining an existing font-family/size/weight) is a strong candidate to pull in as a new composite semantic token even with zero current component consumers, the same way `sizing.*`/`font-size`'s completed steps were added code-first (see `_owner_note`/`_fontSizeNote` in `figma-sync-state.json`). A style property left at Figma's own default (e.g. text style `lineHeight: { unit: 'AUTO' }`, unbound) means Figma itself didn't specify a value — don't invent one on the code side to fill the gap; leave that property out of the token rather than guessing.
...
- Assuming the repo-wide grep only needs to check docs — real shipped CSS (`packages/styles/src/components/*.css` and `packages/web-components/src/global/docs.css`) references token names directly too, and a stale reference there is a live rendering bug, not just documentation drift.
- **Do not create or edit `packages/web-components/styles/`.** It looks like a tracked duplicate of `packages/styles/src/components/*.css` but it isn't — it's untracked (`git status` shows `??`), and `wend-button.tsx`'s `styleUrl` (`../../../../styles/src/components/wend-button.css`) resolves only to `packages/styles/src/components/`, never to this path. It has been deleted twice after silently reappearing because this skill used to instruct fixing it as a "duplicate pair" — `packages/styles/src/components/*.css` is the only real component-CSS source. If it reappears, delete it; don't populate it.
...
- **Adding a new file under `tokens/semantic/*.json` and expecting the build to pick it up automatically.** It won't. `style-dictionary.config.js`/`style-dictionary.dark.config.js`'s `source` arrays list `tokens/semantic/color.json` (and `color.dark.json`) *explicitly* — not a `tokens/semantic/**/*.json` glob like `global`/`component` get — so a brand-new semantic file (e.g. `text-style.json`) builds successfully with zero errors and silently produces none of its tokens in any output. Add the new filename to the `source` array in **both** config files, then rebuild and grep the built CSS for the new token names to confirm they actually landed before moving on.
- Only checking Variables and never Styles when asked to "sync with Figma" or "check for drift" — see the "When to use" note above. A Style with no bound Variables at all (a literal color/size baked directly into the style) is a signal someone authored it without reusing existing tokens — worth flagging to the user rather than silently either adopting the literal or ignoring the style.
