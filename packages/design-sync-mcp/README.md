...
- **`global`** — the primitive scale (`gray-*`/`amber-*` ramps, plus `color-green-600`/`color-red-600` single-shade feedback colors, `radius-*`, `spacing-*`, `font-*`). Single mode (`Value`) — primitives don't change with theme, so there's no Light/Dark split here.
