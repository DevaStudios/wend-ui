...
None of these tools cover Figma **Styles** (Text/Effect/Paint/Grid) — a separate Figma feature from Variables, with its own API (`figma.getLocalTextStylesAsync()` etc.) and no `diff_tokens`-equivalent here. Per the `sync-tokens-to-figma` skill, check Styles by hand alongside Variables whenever auditing Figma for drift or new content — see `figma-sync-state.json`'s `styles` section for what's already been pulled (`text-style.json`, from the 5 local Text Styles) and what's still empty (Effect/Paint/Grid, as of 2026-09-09).
...
- **`global`** — the primitive scale: a 14-step `gray` ramp (`25`–`1000`) plus full 10-step `yellow`/`green`/`red` ramps (`50`–`900`), and `radius-*`/`spacing-*`/`font-*`. Single mode (`Value`) — primitives don't change with theme, so there's no Light/Dark split here. Naming is inconsistent across individual variables — some use slash-style (`color/gray/25`, `color/green/600`), others dash-style (`color-gray-700`, `color-yellow-500`) — a byproduct of this ramp being touched across multiple sync passes; confirm a variable's actual name with a read-only script rather than assuming either style. `yellow` (added 2026-09-08) replaced the old `amber` ramp wholesale — Figma authored it as an entirely new set of variables rather than renaming `amber` in place, so the old `amber` variables are still sitting in the file detached/orphaned from the collection's `variableIds` rather than deleted; harmless but worth a Figma-side cleanup at some point.
- **`semantic`** — everything else: `color-text-*`/`color-surface-*`/`color-border-*`/`color-action-*`/`color-feedback-*`, the component-tier `color-button-*` color tokens, and the component-tier `button-*` non-color tokens (padding/radius/font). Has **Light** and **Dark** modes. Each variable's value is a `VARIABLE_ALIAS` pointing at the `global` collection (or, for `color-button-*`, at another `semantic` variable) rather than a resolved literal — matching how the source JSON itself references tokens (e.g. `color.button.primary.background.default` → `{color.action.primary.background}` → `{color.gray.950}`). This means changing a global primitive in Figma propagates to every semantic/component token that references it, the same as the build pipeline. As of the 2026-09-08 sync, `action.*`/`button.*` color tokens flip by mode like everything else (matching Figma's own dark values) — the earlier "buttons keep a fixed appearance regardless of app theme" convention was dropped. `action.primary.foreground`/`action.destructive.foreground` also flip now (fixed the same day, after a user report caught dark-mode button text rendering wrong); `action.primary.foreground-disabled` is the one leaf still deliberately fixed both modes (see `packages/tokens/README.md`'s Color scale section).
