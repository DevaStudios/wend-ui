...
```
tokens/
  global/      # raw primitives, no meaning — color ramps (gray/blue/purple/green/yellow/red), spacing, sizing, radius, typography
...
As of the 2026-09-08 Figma sync, `action.primary`/`action.secondary`/`action.tertiary`/`action.destructive*` (and therefore every `button.*` color that references one) **flip by mode**, matching Figma's own dark-mode values for each — the earlier "buttons keep a fixed appearance regardless of app theme" design decision was superseded once Figma's dark variants for these roles were pulled in (see `tokens/semantic/color.dark.json`'s `action.*` block). The one exception is `action.primary.foreground`/`action.primary.foreground-disabled` (and the equivalent `action.destructive.foreground`), which still stay fixed both modes (`gray.50`/`gray.500`) — Figma's own dark values for these specific leaves (`gray.25`/`gray.700`) weren't adopted since they'd also change the *light*-mode value, a separate discrepancy from the dark-mode-only drift this sync fixed; flagged for a future decision rather than folded in silently. `text.primary`, `text.secondary`, and `text.disabled` still flip normally between modes (they follow the surrounding page theme).

`yellow`, `green`, and `red` are the feedback colors (warning/success/danger) — each a full 10-step scale (`50`→`900`), unrelated to the neutral `gray` scale. `red` is the most heavily used: `600`/`500` back `color-feedback-danger-background` (light/dark), and the destructive button variants (`destructive-primary`/`destructive-secondary`/`destructive-tertiary`) additionally consume `50`, `100`, `300`, `400`, `500`, `700`, `800`, `900` across the `color-action-destructive*` tokens (light/dark, default/hover) — see [Color token naming](#color-token-naming) below. As of the Help Text / Text Input pull (2026-08-24), `yellow` (then still named `amber`) and `green` were no longer just `500`/`600`: `color-text-success`/`color-border-success` (`green.800` light, `green.300`/`green.400` dark) and `color-text-warning`/`color-border-warning` (`yellow.800`/`yellow.700` light, `yellow.300`/`yellow.400` dark) round out the two ramps, plus `color-text-danger`/`color-border-danger` (`red.700` light, `red.200`/`red.300` dark) for `red`. As of the 2026-09-08 Figma sync, the `amber` ramp was replaced wholesale by `yellow` — Figma authored `yellow` as an entirely new 10-step scale (not a rename of `amber`; the old `amber` variables were left orphaned in Figma), so code swapped every `{color.amber.N}` reference for `{color.yellow.N}` at the same step number and adopted Figma's actual yellow values below.

| Step  | Yellow    | Green     | Red       |
| ----- | --------- | --------- | --------- |
| `50`  | `#FFFBEB` | `#F0F9F5` | `#FAF0EF` |
| `100` | `#FEF4C7` | `#DBF5E9` | `#F7DCD9` |
| `200` | `#FDE88A` | `#BAEED5` | `#F3BBB4` |
| `300` | `#FCD64D` | `#8AE5BB` | `#EE8C81` |
| `400` | `#FBC428` | `#5ADDA0` | `#E95D4E` |
| `500` | `#F5A20B` | `#30D588` | `#E43421` |
| `600` | `#D97B06` | `#27B070` | `#BB2B1B` |
| `700` | `#B45609` | `#238B5A` | `#93261A` |
| `800` | `#92420E` | `#206A47` | `#702219` |
| `900` | `#78370F` | `#1C4A35` | `#4F1D17` |

`green` and `red` were generated in HSL — fixed hue per color (green ≈152°, red ≈6°, calibrated from the original `500`/`600` anchors), a shared lightness curve across both (`96, 91, 83, 72, 61, 51, 42, 34, 27, 20` for `50`→`900`), and per-hue saturation tapering at both extremes; see [`docs/superpowers/specs/2026-08-04-color-ramp-expansion-design.md`](../../docs/superpowers/specs/2026-08-04-color-ramp-expansion-design.md) for the full design rationale (that doc predates the `yellow` swap and still refers to `amber` throughout — historical record, not updated). `yellow` does **not** follow that same fixed-hue formula — it was pulled directly from Figma's own authored values, and its hue actually drifts warmer (≈48° at `50` down to ≈23° at `900`) rather than holding constant like `green`/`red`.

## Color token naming

Semantic and component color tokens follow `color-{scope}-{variant}-{property}[-{state}]`, where `{scope}` is a semantic category (`text`, `surface`, `border`, `action`, `feedback`) or a component name (`button`, `checkbox`, `toggle`, `radio`, `radio-group`, `help-text`, `text-input`, `text-area`):
